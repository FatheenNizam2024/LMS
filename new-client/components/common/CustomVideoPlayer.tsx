...
import { View, Text, StyleSheet, TouchableOpacity, ActivityIndicator, DimensionValue, Modal, Pressable, BackHandler, LayoutChangeEvent  } from 'react-native';
...
const PlayerContent = ({ hlsUrl, username, isFullscreen, onToggleFullscreen }: Props & { isFullscreen: boolean, onToggleFullscreen: () => void }) => {
  ...
  return (
    <View style={styles.videoWrapper}>
        ...
        <Text style={[styles.watermark, { top: watermark.top, left: watermark.left }]}>{username}</Text>
    ...
    return (
        <>
            <View style={styles.container}>
                <PlayerContent hlsUrl={hlsUrl} username={username} isFullscreen={false} onToggleFullscreen={handleToggleFullscreen} />
            </View>
            <Modal visible={isFullscreen} supportedOrientations={['landscape', 'portrait']}>
                {/* THE FIX: Pass the onLayout prop to the View */}
                <View style={styles.fullscreenContainer} onLayout={onLayout}>
                    {screenDimensions.width > 0 && (
                        <View style={{ width: screenDimensions.width, height: screenDimensions.height }}>
                            <PlayerContent hlsUrl={hlsUrl} username={username} isFullscreen={true} onToggleFullscreen={handleToggleFullscreen} />
                        </View>
                    )}
                </View>
            </Modal>
